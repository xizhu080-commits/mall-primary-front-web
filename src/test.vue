在以下代码的基础上,解决规格没有映射成功的问题,以及改变规格和数量时价格不变的问题!
<template>
          ...
          <div v-else>
            ...
            >
              ...
              <div class="address-actions">
                <button class="action-edit" @click.stop="openAddressForm('edit', addr)">
                  编辑
                </button>
...
const payableAmount = computed(() => {
  if (orderPreviewData.value) return orderPreviewData.value.payableAmount
  return currentSku.value.price * quantity.value
})
...
const getSpecLabel = (specName) =>
  ({ color: '颜色', size: '容量', packageType: '套餐类型', area: '适用面积' })[specName] || specName
const getOptionLabel = (specName, value) =>
  ({ color: { 森野绿: '森野绿', 冷烟紫: '冷烟紫' }, size: { '256GB': '256GB', '1TB': '1TB' } })[
    specName
  ]?.[value] || value

const buildSpecList = (specNameStr, skus) => {
  const specNames = specNameStr.split(';').filter((s) => s.trim())
  const specs = []
  for (const specName of specNames) {
    const optionsSet = new Map()
    for (const sku of skus) {
      const specData = parseSpecData(sku.specData)
      let value = null
      if (specName === '颜色' || specName === 'color') value = specData.color
      else if (specName === '容量' || specName === 'size') value = specData.size
      else value = specData[specName] || specData[specName.toLowerCase()]
      if (value && !optionsSet.has(value))
        optionsSet.set(value, { value, label: getOptionLabel(specName, value), stock: sku.stock })
    }
    specs.push({
      name: getSpecLabel(specName),
      key: specName,
      options: Array.from(optionsSet.values()),
    })
  }
  return specs
}
...
const findSkuBySpecs = () => {
  for (const sku of skuList.value) {
    ...
    for (const [specName, specValue] of Object.entries(selectedSpecs)) {
      let skuValue = null
      if (specName === '颜色' || specName === 'color') skuValue = specData.color
      else if (specName === '容量' || specName === 'size') skuValue = specData.size
      else skuValue = specData[specName] || specData[specName.toLowerCase()]
...
const selectSpecOption = async (specName, value) => {
  if (selectedSpecs[specName] === value) delete selectedSpecs[specName]
  else selectedSpecs[specName] = value
  const newSku = findSkuBySpecs()
  if (newSku) {
    currentSku.value = { ...newSku }
    currentImage.value = getValidImageUrl(newSku.productUrl)
    currentSpecParams.value = parseSpecData(newSku.specData)
    if (isLoggedIn.value && selectedCouponIds.value.length > 0) await fetchOrderPreview()
  }
}
...
const isSpecOptionAvailable = (specName, optionValue) => {
  ...
  for (const sku of skuList.value) {
    ...
    for (const [sName, sValue] of Object.entries(tempSpecs)) {
      let skuValue = null
      if (sName === '颜色' || sName === 'color') skuValue = specData.color
      else if (sName === '容量' || sName === 'size') skuValue = specData.size
      else skuValue = specData[sName] || specData[sName.toLowerCase()]
      if (skuValue !== sValue) {
        match = false
        break
      }
...
const initSelectedSpecs = () => {
  ...
  for (const spec of specList.value) {
    if (spec.options.length > 0 && !selectedSpecs[spec.name])
...
const increaseQty = async () => {
  if (quantity.value < currentSku.value.stock) {
    quantity.value++
    if (selectedCouponIds.value.length > 0) await fetchOrderPreview()
  }
}
...
const checkPaymentStatus = () => {
  console.log('检查支付状态,订单ID:', currentOrderId)
  showToast('支付成功！')
  closeQrModal()
  showPaymentSuccess()
}
...
const startExpireTimer = () => {
  ...
  expireTimer = setInterval(() => {
    if (expireSeconds.value > 0) {
      expireSeconds.value--
      console.log('二维码剩余时间:', expireSeconds.value, '秒')
    } else {
...
const createPayment = async (orderId) => {
  ...
  try {
    ...
    if (res.code === 200 && res.data) {
      ...
      console.error('创建支付单失败:', res.msg || res.message)
...
const refreshQrCode = async () => {
  if (!currentOrderId) {
    console.error('订单ID不存在，无法重新获取二维码')
    showToast('订单信息错误，请重新下单')
    closeQrModal()
    return
  }

  console.log('开始重新获取二维码，订单ID:', currentOrderId)
  refreshing.value = true

  try {
    const payType = selectedPayment.value === 'alipay' ? 'Alipay' : 'Balance'
    const requestData = { orderId: currentOrderId, payType }
    console.log('重新获取支付单请求参数:', requestData)

    const res = await payOrder(requestData)
    console.log('重新获取支付单响应:', res)
    ...
    if (res.code === 200 && res.data) {
      ...
      isSuccess = true
    } else if (res.orderId && res.qrCode) {
      responseData = res
      isSuccess = true
    } else if (res.qrCode) {
      responseData = res
    ...
    if (isSuccess && responseData) {
      ...
      console.log('新获取的二维码数据:', qrData ? qrData.substring(0, 100) + '...' : 'null')
      ...
      if (qrData) {
        ...
        console.log('二维码已刷新，新倒计时开始')
      } else {
        console.error('二维码数据为空')
        showToast('获取二维码失败，请重试')
      }
    } else {
      console.error('重新获取支付单失败:', res.msg || res.message || res)
      showToast(res.msg || '获取二维码失败，请重试')
    }
  } catch (error) {
    console.error('重新获取二维码异常:', error)
    showToast('网络错误，请重试')
  } finally {
    refreshing.value = false
  }
...
const submitOrder = async () => {
  ...
  console.log('开始提交订单，收货信息:', { receiver, phone, address })
  console.log('商品信息:', { skuId: currentSku.value.skuId, quantity: quantity.value })
  console.log('选中的优惠券:', selectedCouponIds.value)
  ...
  try {
    ...
    console.log('创建订单请求参数:', requestData)
    ...
    if (res.code === 200 && res.data && res.data.orderId) {
      ...
      console.log('订单创建成功，订单ID:', currentOrderId)
      await createPayment(currentOrderId)
    } else {
      console.error('创建订单失败:', res.msg || res.message)
...
// 删除 initPaymentWebSocket 调用，改用全局事件监听
...
<style scoped>
/* 原有样式保持不变，这里只列出需要修改的关键样式 */

/* 移除遮罩层的点击关闭功能 - 注释掉原有的 @click.self */
/* .modal-overlay 不再需要 @click.self 事件 */
